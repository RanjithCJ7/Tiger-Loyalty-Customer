...
import 'package:tl_customer/screens/rewards/component/rewards_screen.dart';
...
class BottomTabController extends GetxController {
  ...
  List<Widget> screen = [RewardsScreen(), Brands(), Promo(), Profile()];
  ...
  Future<bool> makePayment(
    ...
    var request = http.Request('POST', Uri.parse(Urls.makePayment));
  ...
  Future<bool> verifyPin(String amount) async {
    ...
    request.body = json.encode({"pin": pinController.text.trim()});
  ...
  Future<bool> verifyCashPin() async {
    isLoadingMerchant(true);
    var request = http.Request('POST', Uri.parse(Urls.verifyPin));
    request.headers.addAll({
      'Content-Type': 'application/json',
  ...
  Future saveReward(String amount) async {
    try {
      ...
      if (response.statusCode == 200) {
        print("save rewards ==> ${result}  ${request.body}");
        rewardsController.getRewardsPoints();
        rewardsController.getRewardSummary("");
        rewardsController.selectedCategory = 'business_category'.obs;
        submitReviewBottomSheet(false);
  ...
  Future redeemPoints(String id, String points) async {
    try {
                              ...
                              children: [
                                ...
                                SizedBox(
                                  ...
                                  child: TextButton(
                                    onPressed: () {
                                      // Get.close(1);
                                      submitReviewBottomSheet(true);
                                    },
                                    style: btnGrey,
                                    child: Text('review_service'.tr,
                                        style: label_sm2),
                                  ),
                                ),
                                const SizedBox(height: 50),
                    ...
                    SizedBox(
                      ...
                      child: TextButton(
                        onPressed: () {
                          ...
                          } else if (rating.value == 0.0) {
                            Fluttertoast.showToast(msg: "select_rating".tr);
                          } else {
                            postReview().then(
